...
from segall.transforms import functional


@manager.TRANSFORMS.add_component
class RGBDSampleRescale:
...
@manager.TRANSFORMS.add_component
class RGBDSampleRandomCrop:
    def __init__(self, crop_height, crop_width):
        self.crop_height = crop_height
        self.crop_width = crop_width
        self.rescale = RGBDSampleRescale(self.crop_height, self.crop_width)
    ...
    def __call__(self, sample):
        ...
        if self._depth_mode == 'raw':
            ...
            depth = normalize(depth,
                mean=self._depth_mean, std=self._depth_std)
...
@manager.TRANSFORMS.add_component
class RGBDSampleMultiScaleLabel:
    ...
    def __call__(self, sample):
        ...
        for rate in self.downsampling_rates:
            label_down = functional.resize(label, (w // rate, h // rate),

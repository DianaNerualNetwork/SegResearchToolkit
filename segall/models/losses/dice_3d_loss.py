...
class DiceLoss3D(nn.Module):
    ...
    def compute_per_channel_dice(self, input, target, epsilon=1e-6,
        ...
        return 2 * (intersect / torch.clip(denominator, min=epsilon))
    ...
    def forward(self, logits, labels):
        ...
        labels_one_hot = F.one_hot(
            labels.to(torch.int64), num_classes=logits.shape[1])  # [B, D, H, W, C]
        # [0,2,1,3,4]==>[0,3,1,2,4]==>[0, 4, 1, 2, 3]
        labels_one_hot=torch.transpose(labels_one_hot,1,2).transpose(1,3).transpose(1,4) 
        labels_one_hot = torch.as_tensor(labels_one_hot, dtype=torch.float32)

...
class BCELoss(nn.Module):
            ...
            if isinstance(self.weight, str):
                if self.weight != 'dynamic':
                    raise ValueError(
                        ...
                        .format(self.weight))
            elif not isinstance(self.weight, torch.Tensor):
                raise TypeError(
        ...
        if self.pos_weight is not None:
            if isinstance(self.pos_weight, str):
                ...
                self.pos_weight = torch.as_tensor(
                    self.pos_weight, dtype=torch.float32)
            else:
                raise TypeError(
    ...
    def forward(self, logit, label):
        ...
        mask = torch.as_tensor(mask, torch.float32)
        ...
        if isinstance(self.weight, str):
            pos_index = (label == 1)
            neg_index = (label == 0)
            pos_num = torch.sum(pos_index.astype('float32'))
            neg_num = torch.sum(neg_index.astype('float32'))
        ...
        loss = loss * mask
        loss = torch.mean(loss) / (torch.mean(mask) + self.EPS)
        label.required_grad = True
        mask.required_grad = True

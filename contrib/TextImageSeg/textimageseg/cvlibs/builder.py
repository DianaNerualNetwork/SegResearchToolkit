...
from textimageseg.cvlibs import manager,Config
from textimageseg.utils import utils,logger
from textimageseg.utils.utils import CachedProperty as cached_property
...
class SegBuilder(Builder):
    ...
    @cached_property
    def lr_scheduler(self) -> torch.optim.lr_scheduler:
    ...
    #     #     end_lr = lr_cfg['learning_rate']
        ...
        if lr_type == 'PolynomialLR':
            ...
            lr_cfg.setdefault('total_steps', self.config.iters)
            lr_cfg.setdefault('steps_per_epoch', iters_per_epoch)
            lr_cfg.setdefault('epochs', epochs) 
            lr_cfg.setdefault('div_factor', 25)
